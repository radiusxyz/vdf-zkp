...
impl<E: Engine> Bitvector<E> {
  ...
  pub fn get(&self, i: usize) -> Option<Bit<E>> {
    self.bits.get(i).map(|lc| Bit { bit: lc.clone(), value: self.values.as_ref().map(|vs| vs[i].clone()) })
  }
  ...
  pub fn into_bits(mut self) -> Vec<Bit<E>> {
    ...
    self.bits.into_iter().enumerate().map(|(i, b)| Bit { bit: b, value: vs.as_ref().map(|vs| vs[i]) }).collect()
  }
}
...
impl<E: Engine> Display for Bitvector<E> {
  fn fmt(&self, f: &mut Formatter) -> fmt::Result {
    match self.values.as_ref() {
      Some(vs) => write!(f, "Bitvector({})", vs.into_iter().map(|b| if *b { "1" } else { "0" }).collect::<Vec<_>>().concat()),
