...
pub trait Gadget: Sized + Clone {
  ...
  type Params: Clone;
  fn alloc<CS: ConstraintSystem<Self::E>>(cs: CS, value: Option<&Self::Value>, access: Self::Access, params: &Self::Params) -> Result<Self, SynthesisError>;
  ...
  fn mux<CS: ConstraintSystem<Self::E>>(mut cs: CS, s: &Bit<Self::E>, i0: &Self, i1: &Self) -> Result<Self, SynthesisError> {
    ...
    let out: Self = Self::alloc(cs.namespace(|| "out"), value, i0.access().clone(), i0.params())?;
  ...
  fn mux_tree<'a, CS: ConstraintSystem<Self::E>>(mut cs: CS, mut select_bits: impl Iterator<Item = &'a Bit<Self::E>> + Clone, inputs: &[Self]) -> Result<Self, SynthesisError> {
  ...
  fn assert_equal<CS: ConstraintSystem<Self::E>>(mut cs: CS, i0: &Self, i1: &Self) -> Result<(), SynthesisError> {
    let i0_wires = i0.wires();
    let i1_wires = i1.wires();
    if i0_wires.len() != i1_wires.len() {
      return Err(SynthesisError::Unsatisfiable);
